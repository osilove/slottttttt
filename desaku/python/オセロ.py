...
class OthelloGame:
    ...
    def create_widgets(self):
        ...
        for row in range(BOARD_SIZE):
            for col in range(BOARD_SIZE):
                button = tk.Canvas(self.board_frame, width=60, height=60, bg="green", bd=0, highlightthickness=0)
                button.grid(row=row, column=col, padx=3, pady=3)
                # 丸いボタンを描くために、canvasに円を描く
                button.create_oval(10, 10, 50, 50, outline="black", width=2)
                button.bind("<Button-1>", lambda event, r=row, c=col: self.cell_click(r, c))
    ...
    def update_board(self):
        for row in range(BOARD_SIZE):
            for col in range(BOARD_SIZE):
                ...
                if piece == BLACK:
                    button.create_oval(10, 10, 50, 50, fill="black", outline="black", width=2)
                elif piece == WHITE:
                    button.create_oval(10, 10, 50, 50, fill="white", outline="black", width=2)
                else:
                    button.create_oval(10, 10, 50, 50, fill="green", outline="black", width=2)
    ...
    def has_valid_moves(self, player):
        for row in range(BOARD_SIZE):
            for col in range(BOARD_SIZE):
                # can_place_piece を無視してどこにでも置けるようにする
                if self.board[row][col] is None:  # 空いている場所であれば常に置ける
                    return True
        return False
